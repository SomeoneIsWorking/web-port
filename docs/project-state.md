...
| S005 | Explicit offline release packaging | partial | Exact input map, content version and unowned-output refusals pass two tests. Lucent worker reached isolated reload on a host without isolation headers and after the server stopped. Real consumer offline gameplay remains. |
